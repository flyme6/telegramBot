...
var ENV = {
  ...
  /**
   * 发送消息到 Telegram 平台的函数。
   * @param {*} message - 要发送的消息内容。
   * @param {*} token - 当前 bot 的 token。
   * @param {*} context - 发送消息的上下文，包括目标聊天室(chat_id)和消息 id(message_id)等。
   * @returns 返回发送消息的 Response 对象。
   **/
  async function sendMessage(message, token, context) {
    ...
    // 将 context 里除 null 和 undefined 以外的键值对添加到 body 中。
    ...
    // 根据是否带有 message_id 判断是发送新消息还是编辑已有的消息。
    let method = "sendMessage";
    if (context?.message_id) {
      method = "editMessageText";
    }
  
    // 发送 POST 请求到 Telegram API ，包含需要发送的消息和目标聊天室信息。
    return await fetch(
  ...
  /**
   * 给定发送消息的上下文 context ，发送消息到 Telegram 平台的函数。
   * @param {*} message - 要发送的消息内容。
   * @param {*} token - 当前 bot 的 token。
   * @param {*} context - 发送消息的上下文，包括目标聊天室(chat_id)和消息 id(message_id)等。
   * @returns 返回发送消息的 Response 对象。
  ...
  async function sendMessageToTelegram(message, token, context) {
    ...
    // 返回一个含有 status 200 的 Response 对象。
    return new Response("Message batch send", { status: 200 });
  }
  
  /**
   * 以发送消息上下文 context 为参数，返回一个发送消息到 Telegram 平台的函数。
   * @param {*} context - 发送消息的 context 上下文。
   * @returns 返回一个发送消息到 Telegram 平台的函数。
   **/
  ...
  /**
   * 以发送消息上下文 context 为参数，返回一个从聊天室中删除消息的函数。
   * @param {*} context - 发送消息的 context 上下文。
   * @returns 返回一个从聊天室中删除消息的函数。
   **/
  function deleteMessageFromTelegramWithContext(context) {
    return async (messageId) => {
  
      // 发送 POST 请求到 Telegram API ，删除指定 messageId 对应的消息。
      return await fetch(
  ...
  /**
   * 发送图片到 Telegram 平台的函数。
   * @param {*} url - 要发送图片的 URL。
   * @param {*} token - 当前 bot 的 token。
   * @param {*} context - 发送图片的上下文，包括目标聊天室(chat_id)和消息 id(message_id)等。
   * @returns 返回发送图片的 Response 对象。
   **/
  async function sendPhotoToTelegram(url, token, context) {
    ...
    // 发送 POST 请求到 Telegram API ，包含需要发送的图片和目标聊天室信息。
    return await fetch(
  ...
  /**
   * 以发送消息上下文 context 为参数，返回一个将图片发送到 Telegram 平台的函数。
   * @param {*} context - 发送消息的 context 上下文。
   * @returns 将图片发送到 Telegram 平台的函数。
   **/
  ...
  /**
   * 向指定聊天室发送 chat action ，如正在输入、上传文件等。
   * @param {*} action - 要发送的 chat action 类型。
   * @param {*} token - 当前 bot 的 token。
   * @param {*} chatId - 目标聊天室的 ID。
   * @returns 返回发送 chat action 的 Response 对象。
   **/
  async function sendChatActionToTelegram(action, token, chatId) {
    // 发送 POST 请求到 Telegram API ，完成 chat action 操作。
    return await fetch(
  ...
  /**
   * 以发送消息上下文 context 为参数，返回一个向指定聊天室发送 chat action 的函数。
   * @param {*} context - 发送消息的 context 上下文。
   * @returns 返回一个向指定聊天室发送 chat action 的函数。
   **/
   ...
   * 绑定当前 bot 的 WebHook 。
   * @param {*} token - 当前 bot 的 token。
   * @param {*} url - WebHook 的 url。
   * @returns 返回绑定 WebHook 的 Response 对象。
   **/
  async function bindTelegramWebHook(token, url) {
    // 发送 POST 请求到 Telegram API ，完成 WebHook 绑定操作。
    return await fetch(
   ...
   * 获取指定用户在指定聊天室的角色。
   * @param {*} id - 要查询的用户 id。
   * @param {*} groupAdminKey - 用于保存聊天室管理员信息的 key 值。
   * @param {*} chatId - 要查询的聊天室 id。
   * @param {*} token - 当前 bot 的 token。
   * @returns 返回指定用户在指定聊天室的角色。
   **/
  async function getChatRole(id, groupAdminKey, chatId, token) {
    ...
    // 如果不存在 groupAdmin 或者 groupAdmin 不是一个数组（因为保存在 groupAdminKey 中的实际上是一个数组），则调用 getChatAdminister 函数获取当前聊天室的管理员信息。
    ...
    // 遍历 groupAdmin 数组，找到目标用户并返回其状态信息。
  ...
  /**
   * 以发送消息上下文 context 为参数，返回一个查询指定用户在指定聊天室中角色的函数。
   * @param {*} context - 发送消息的 context 上下文。
   * @returns 返回一个查询指定用户在指定聊天室中角色的函数。
   **/
   ...
   * @param {*} chatId - 要查询的聊天室 id。
   * @param {*} token - 当前 bot 的 token。
   * @returns 返回指定聊天室的管理员列表。
   **/
  async function getChatAdminister(chatId, token) {
    try {
      // 发送 POST 请求到 Telegram API ，获取当前聊天室的管理员列表。
  ...
  // 定义一个异步函数 getBot，接收一个 token 参数
  async function getBot(token) {
    // 使用 fetch 方法向 Telegram API 发送请求
    // ${ENV.TELEGRAM_API_DOMAIN} 是Telegram API的域名
    // /bot${token}/getMe 是获取机器人信息的API路径
    // 发送一个 POST 请求，并设置请求头 Content-Type 为 application/json
    const resp = await fetch(
    ...
    if (resp.ok) {
      ...
      return {
        ok: true,
        info: {
          name: resp.result.first_name, // 机器人的名字
          bot_name: resp.result.username, // 机器人的用户名
          can_join_groups: resp.result.can_join_groups, // 是否能加入群组
          can_read_all_group_messages: resp.result.can_read_all_group_messages // 是否能获取所有群组的信息
        }
      };
    } else {
      // 如果失败，返回响应本身
  ...
  // src/gpt3.js
  /**
   * 异步函数，用于从数据库或 URL 中加载资源，如果在数据库中找到资源，则获取并返回该资源。否则从 URL 中获取资源并将其存储到数据库，然后返回资源。
   * @param {string} key - 数据库中标识资源的键。
   * @param {string} url - 要加载资源的 URL。
   * @returns {Promise<string|null>} - 加载成功后，将返回加载的资源的字符串表示形式，如果加载失败，将返回 null。
   */
  async function resourceLoader(key, url) {
    try {
      const raw = await DATABASE.get(key);
      if (raw && raw !== "") {
        // 如果在数据库中找到了资源，直接返回该资源
        return raw;
      }
    } catch (e) {
      console.error(e);
    ...
    try {
      // 如果在数据库中没有找到资源，则从 URL 中获取资源
      const bpe = await fetch(url, {
        headers: {
          // 设置请求头中的 User Agent
          "User-Agent": CONST.USER_AGENT
        }
        // 通过 fetch API 获取资源的文本数据
      }).then((x) => x.text());
      // 将资源存储到数据库
  ...
  async function gpt3TokensCounter() {
    ...
    // 返回 {x: y} 形式的字典，其中 x 和 y 分别为两个数组中相同索引处的元素
    const dictZip = (x, y) => {
      const result = {};
      x.map((_, i) => {
        result[x[i]] = y[i];
    ...
    // 根据所需的 Unicode 范围生成字典，将每个字节映射到其对应的 Unicode 字符
    ...
    // 对给定 token 进行 BPE 算法编码，并返回编码后的字符串
    ...
    // 返回接受字符串参数并返回数字的函数，用于统计给定文本中 GPT-3 Token 的数量
    return function tokenCount(text) {
      let tokensCount = 0;
  
      // 使用正则表达式将给定文本拆分成多个 token，并进行 BPE 编码和计数
  ...
  // 渲染 HTML 页面
  // @param {string} body - HTML 页面的主体内容
  // @returns {string} - 渲染后的完整 HTML 页面
  ...
  // 将响应对象转换为状态码为 200 的响应对象
  // @param {Response} resp - 原始响应对象
  // @returns {Response} - 转换后的响应对象
  ...
  /**
   * 从流数据中提取内容
   * @param {string} stream - 流数据
   * @returns {object} - 包含提取的内容和剩余字符串的对象
   */
   ...
   * @param {Function} onStream - 流式处理回调函数
   * @returns {string} - OpenAI 返回的生成文本
  ...
  async function requestCompletionsFromOpenAI(message, history, context, onStream) {
    ...
    // 如果使用的是 Azure 完成 API，则使用对应的 URL 和头部信息
    if (ENV.AZURE_COMPLETIONS_API) {
    ...
    // 发送 POST 请求到指定的 URL，并提供 headers、body 和 signal
    ...
    // 如果 onStream 存在，并且响应成功，并且响应的内容类型是 "text/event-stream"，则使用流式处理读取响应体的数据
    if (onStream && resp.ok && resp.headers.get("content-type").indexOf("text/event-stream") !== -1) {
      ...
      while (data.done === false) {
        try {
          ...
          // 如果 lengthDelta 大于 updateStep，则调用 onStream 函数，并传递 contentFull 和加载中的提示
  ...
  /**
   * 从 OpenAI 请求图片的方法
   * @param {string} prompt - 图片的提示
   * @param {Object} context - 上下文对象
   * @returns {string} - OpenAI 返回的生成图片的 URL
   * @throws {Error} - 请求错误时抛出异常
   */
  async function requestImageFromOpenAI(prompt, context) {
    ...
    // 发送 POST 请求到指定的 URL，并提供 headers 和 body
   ...
   * @param {Object} context - 上下文对象
   * @param {Function} modifier - 修改器函数，用于修改历史记录和文本
   * @param {Function} onStream - 数据流回调函数
   * @returns {string} - OpenAI 返回的生成文本
   */
  async function requestCompletionsFromChatGPT(text, context, modifier, onStream) {
    ...
    // 如果存在修改器函数，则使用修改器函数对历史记录和文本进行修改
    if (modifier) {
    ...
    // 如果未禁用历史记录，则将用户输入和生成的文本添加到原始历史记录中，并保存到数据库
    if (!historyDisable) {
  ...
  /**
   * 加载历史记录的方法
   * @param {string} key - 历史记录的键名
   * @param {Object} context - 上下文对象
   * @returns {Object} - 包含实际历史记录和原始历史记录的对象
   */
  async function loadHistory(key, context) {
    ...
    // 如果历史记录不存在或不是数组类型，则将历史记录数组初始化为空数组
   ...
   * @param {string} text - 用户输入的文本
   * @param {Object} context - 上下文对象
   * @param {Function} modifier - 修改器函数
   * @returns {Promise} - 发送消息的 Promise 对象
   */
  async function chatWithOpenAI(text, context, modifier) {
    try {
      // 尝试发送加载中的消息到 Telegram
      try {
      ...
      if (ENV.STREAM_MODE) {
        ...
        // 定义 onStream 回调函数，用于发送消息到 Telegram
   ...
   * @param {string} subcommand - 子命令字符串
   * @param {Object} context - 上下文对象
   * @returns {Promise} - 返回发送消息的 Promise 对象
   */
  ...
  async function commandGenerateBill(message, command, subcommand, context) {
    // 调用 requestBill 函数获取账单信息
    const bill = await requestBill(context);
  
    // 生成账单详情消息
    const billDetail = ENV.I18N.command.bill.bill_detail(
      bill.totalAmount,
      bill.totalUsage,
      bill.remaining
    );
    ...
    // 发送消息到 Telegram
    return sendMessageToTelegramWithContext(context)(billDetail);
  }
   ...
   * @param {Object} message - Telegram 消息对象
   * @param {Object} context - 上下文对象
   * @returns {Promise|null} - 返回发送消息的 Promise 对象或 null
   */
   ...
   * 绑定 Telegram 命令
   *
   * @param {string} token - Telegram bot 的访问令牌
   * @returns {Promise<Object>} - 返回一个包含绑定结果的 Promise 对象
   */
   ...
   * @param {object} context - 聊天上下文对象
   * @returns {Promise<Response|null>} - 返回一个包含错误信息的 Promise 对象或者 null
   */
   ...
   * @param {object} message - 接收到的消息对象
   * @param {object} context - 聊天上下文对象
   * @returns {Promise<Response|null>} - 返回一个表示保存结果的 Promise 对象或者 null
   */
   ...
   * @param {object} message - 接收到的消息对象
   * @param {object} context - 聊天上下文对象
   * @returns {Promise<Response|null>} - 返回一个表示处理结果的 Promise 对象或者 null
   */
   ...
   * @param {object} message - 接收到的消息对象
   * @param {object} context - 聊天上下文对象
   * @returns {Promise<Response|null>} - 返回一个表示错误信息的 Promise 对象或者 null
   */
   ...
   * @param {string} text - 用户输入的文本
   * @param {object} context - 聊天上下文对象
   * @param {object|null} options - 聊天选项（可选）
   * @returns {Promise<any>} - 返回一个包含聊天响应的 Promise 对象
   */
  async function msgChatWithOpenAI(text, context, options) {
    return chatWithOpenAI(text, context, options);
  }
   ...
   * @param {object} context - 聊天上下文对象
   * @returns {Promise<object>} - 返回一个包含消息内容的 Promise 对象
   */
  async function loadMessage(request, context) {
    const raw = await request.json();
  
    // 在开发模式下，将原始消息存储到数据库中以进行日志记录
    if (ENV.DEV_MODE) {
      setTimeout(() => {
  ...
  `;
  /**
   * 构建缺失环境变量的错误信息的 HTML 字符串
   *
   * @param {string} key - 缺失的环境变量的名称
   * @returns {string} - 返回构建的 HTML 字符串
   */
  ...
  /**
   * 绑定Telegram的Webhook操作
   *
   * @param {Request} request - 请求对象
   * @returns {Response} - 返回用于响应的Response对象
   */
   ...
   *
   * @param {Request} request - 请求对象
   * @returns {Response|null} - 返回用于响应的Response对象或null
   */
